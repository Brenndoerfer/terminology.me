                ...
                {props.terms && props.terms.map((term) => ((
                    <div className="xl:w-1/4 md:w-1/2 p-4" key={term.title}>
                        <Link href={`${TERM_PATH}/[slug]`} as={`${TERM_PATH}/${term.slug}`} shallow={false}><a>
                            <div className={overrideTailwindClassNames(classNames("bg-white p-4 rounded-md border shadow-sm hover-fade", "border-gray-100", 'hover:' + domainShortToColor(term.domainShort).mediumBorder))}>
                                <h3 className={overrideTailwindClassNames(classNames("tracking-wied text-xs font-medium uppercase", domainShortToColor(term.domainShort).text))}>{term.domain}</h3>
